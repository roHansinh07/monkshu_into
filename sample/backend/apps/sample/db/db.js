...
const getPool = () => {
    return new Promise((resolve, reject) => {
        try {
            if (!mysqlConfig) return reject("Db configuration file is not setup properly.");

            const pool = mysql.createPool(mysqlConfig);
            // Ping database to check for common exception errors.
            pool.getConnection((error, connection) => {
                if (error) {
                    if (error.code === "PROTOCOL_CONNECTION_LOST") console.error("Database connection was closed.");
                    if (error.code === "ER_CON_COUNT_ERROR") console.error("Database has too many connections.");
                    if (error.code === "ECONNREFUSED") console.error("Database connection was refused.");
                    return reject(error);
                }
...
const getConnection = () => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!pool) return reject("Unable to get pool inside getConnection function.");
            pool.getConnection((error, connection) => {
...
const releaseConnection = (connection) => {
    ...
    connection.destroy(); return true;
};
...
const beginTransaction = () => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!connection) return reject("Connection not found inside beginTransaction function.");
...
const commit = (connection) => {
    return new Promise((resolve, reject) => {
        try {
            if (!connection) return reject("Connection not found inside commit function.");
...
const consecutiveSelect = (connection, query, queryParams) => {
    return new Promise((resolve, reject) => {
        try {
            if (!connection) return reject("Connection not found inside consecutiveSelect function.");
            const queryStats = connection.query(query, queryParams, (error, results) => {
                // error will be an Error if one occurred during the query
                if (error) return reject(error);
                if (!results || results.length == 0) return resolve(false);
...
const consecutiveInsert = (connection, tableName, queryParams) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!connection) return reject("Connection not found inside consecutiveInsert function.");
            const query = `INSERT INTO ${tableName} SET ?`;
            // Use the connection
            const queryStats = connection.query(query, queryParams, (error, results) => {
                // error will be an Error if one occurred during the query
...
const consecutiveUpdate = (connection, query, queryParams) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!connection) return reject("Connection not found inside consecutiveUpdate function.");
            // Use the connection
            const queryStats = connection.query(query, queryParams, (error, results) => {
                // error will be an Error if one occurred during the query
                if (error) return reject(error);
...
module.exports = {
    getPool, getConnection, rollBack, beginTransaction, commit, simpleSelect,
    consecutiveSelect, simpleInsert, consecutiveInsert, simpleUpdate, consecutiveUpdate
};